...
const monsters = {
  ...
  2: {
    ...
    hint: (
      <p>
        Помимо прочего, путник рассказал много интересных вещей - например, что у местных плотников есть чудо-пластина, стоит взглянуть через неё на конструкцию - сразу видно, как нагрузка в ней распределена. Найти их теперь легко - как раз на днях соорудили себе красивую вывеску с драконом!
      </p>
    ),
  ...
  3: {
    ...
    hint: (
      <p>
        Магистры Ордена Оптики мастерски владели такой магией, и помогали им в этом небольшие и очень простые артефакты, которые, к тому же, они использовали, когда готовились к важным церемониям и облачались в ритуальные костюмы. К сожалению, они не любят раскрывать свои секреты, но, уверен, ты сможешь найти артефакт, который носит с собой любой уважающий себя Магистр.
      </p>
    ),
  ...
  5: {
    ...
    hint: (
      <p>
        Такое ощущение, будто контуры монстра разделены на два канала, синий и красный... Кажется, Магитры Ордена Оптики носили специальные приспособления, которые помогали объединять такие каналы и поэтому они невосприимчивы к такой магии.
      </p>
    ),
  ...
  6: {
    ...
    hint: (
      <p>
        Говорят, в резиденции Ордена Оптики есть специальная комната, освещение которой делает невидимое видимым. Возможно именно в этом месте тебе удастся сразиться со своим противником на равных!
      </p>
    ),
  ...
  7: {
    ...
    hint: (
      <p>
        Ты слышал, что Ордена Оптики хранит артефакт, способный освещать предметы без источников света. Звучит как что-то, что могло бы здорово выручить в твоём положении!
      </p>
    ),
  ...
  8: {
    ...
    hint: (
      <p>
        Магистры Ордена Оптики владеют искусством переходить на сторону теней с помощью магической стены в резиденции. Чтобы сразиться с этим противником, нужно её отыскать.
      </p>
    ),
